...
class EngineSimple:
    ...
    def __init__(self):
        ...
        self.diag_interval_sec = float(os.getenv("DIAG_INTERVAL_SEC", "60"))
        self._last_diag_emit_ts = 0.0
        self._score_samples: Deque[Tuple[float, float]] = deque(maxlen=12000)
        self._buy_samples: Deque[Tuple[float, float, float, float, float]] = deque(maxlen=2000)
    ...
    def _quantile(self, vals: list[float], q: float) -> float:
        if not vals:
            return 0.0
        arr = sorted(vals)
        idx = int((len(arr) - 1) * min(1.0, max(0.0, q)))
        return arr[idx]

    def _gate_key(self, why: str) -> str:
        if why == "watchlist_out":
            return "watchlist_out"
        if why == "already_held":
            return "already_held"
        if why.startswith("max_positions"):
            return "max_positions"
        if why.startswith("cooldown"):
            return "cooldown"
        if why.startswith("dayrise_hard"):
            return "dayrise_hard"
        if why == "trv10_too_low":
            return "trv10_too_low"
        if why.startswith("score<"):
            return "score_below_threshold"
        if why == "cash_short":
            return "cash_short"
        return "other"

    def _emit_diag_rollups(self, ts_epoch: float):
        if self.diag_interval_sec <= 0:
            return
        if (ts_epoch - self._last_diag_emit_ts) < self.diag_interval_sec:
            return
        self._last_diag_emit_ts = ts_epoch

        cutoff = ts_epoch - max(5.0, self.diag_interval_sec)
        while self._score_samples and self._score_samples[0][0] < cutoff:
            self._score_samples.popleft()
        while self._buy_samples and self._buy_samples[0][0] < cutoff:
            self._buy_samples.popleft()

        scores = [s for _, s in self._score_samples]
        pass_n = sum(1 for s in scores if s >= self.entry_score_threshold)
        if scores:
            self._log_diag(
                ts_epoch,
                "-",
                "SCORE_DIAG",
                f"score_diag n={len(scores)} avg={sum(scores)/len(scores):.1f} p50={self._quantile(scores,0.5):.1f} p90={self._quantile(scores,0.9):.1f} max={max(scores):.1f} pass={pass_n}",
            )
        ...
        self._log_diag(
            ...
            "GATE_DIAG",
            "gate_diag "
            f"watchlist_out={gc.get('watchlist_out',0)} held={gc.get('already_held',0)} "
            f"maxpos={gc.get('max_positions',0)} cooldown={gc.get('cooldown',0)} "
            f"dayrise={gc.get('dayrise_hard',0)} trv10={gc.get('trv10_too_low',0)} "
            f"score={gc.get('score_below_threshold',0)} cash={gc.get('cash_short',0)}",
        )

        if self._buy_samples:
            n = len(self._buy_samples)
            avg_score = sum(x[1] for x in self._buy_samples) / n
            avg_dayrise = sum(x[2] for x in self._buy_samples) / n
            avg_trv10 = sum(x[3] for x in self._buy_samples) / n
            avg_ofi = sum(x[4] for x in self._buy_samples) / n
            self._log_diag(
                ts_epoch,
                "-",
                "BUY_DIAG",
                f"buy_diag n={n} avg_score={avg_score:.1f} avg_dayrise={avg_dayrise:.2f} avg_trv10={avg_trv10:.0f} avg_ofi={avg_ofi:.2f}",
            )
    ...
    def enter_position(self, sym: str, price: float, score: float, reasons: list[str], metrics: Dict[str, float], ts_epoch: float):
        ...
        if qty <= 0:
            ...
            self._gate_counts["cash_short"] += 1
        ...
        self._buy_samples.append((
            ts_epoch,
            score,
            float(metrics.get("dayrise", 0.0)),
            float(metrics.get("trv10", 0.0)),
            float(metrics.get("ofi", 0.0)),
        ))
        self.notifier.send(
    ...
    def on_trade(self, row: Dict[str, str], ts_epoch: float):
        ...
        self._score_samples.append((ts_epoch, score))
        ...
        if not ok:
            ...
            self._gate_counts[self._gate_key(why)] += 1
            self._emit_diag_rollups(ts_epoch)
            return

...
def fetch_rank(market: str) -> List[Dict[str, Any]]:
    """Fetch top-30 'traded by company' ranking for a given market code.
    market: 'J' (KRX) or 'NX' (NXT) as per doc.
    """
    d = _today_yyyymmdd()
    ...
    params = {
        ...
        "fid_rank_sort_cls_code": os.getenv("FID_RANK_SORT_CLS_CODE", "1"),  # 0 sell / 1 buy
...
def build_watchlist() -> List[str]:
    """Build watchlist from ranking API results (max 30 per market).
    Applies basic filters:
      - block day rise >= ENTRY_BLOCK_DAYRISE_PCT (default 12)
      - require trade value >= WATCH_MIN_TR_VALUE (default 3e8)
    """
    want_n = int(os.getenv("WATCH_TOP_N", "30"))
    min_tv = float(os.getenv("WATCH_MIN_TR_VALUE", "300000000"))  # 3억
    ...
    for m in markets:
        for it in fetch_rank(m):
            sym = it.get("mksc_shrn_iscd") or it.get("MKSC_SHRN_ISCD") or it.get("code")
            if not sym:
                continue
            sym = str(sym).zfill(6)
            ...
            # score: trade value primary + small bonus for rise
            score = tv + (r * 1e7)
